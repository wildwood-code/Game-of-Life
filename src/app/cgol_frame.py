...
#
#    This file implements the CGOL_frame object, based upon wxPython wx.Frame.
#
#  Change log:
...
class CGOL_frame(frmMain):
    ...
    def __init__(self, parent, filename:str|None=None):
        """CGOL constructor
        ...
        self.pnlGrid.configure(color=CGOL_frame.LIVE_CELL_COLOR, cell_wh=CGOL_frame.CELL_WH, highlight=CGOL_frame.HIGHLIGHT_COLOR)
        self.pnlGrid.set_message_handler(self.show_message)
        self.pnlGrid.set_status_update_handler(self.update_status_bar)
        self.pnlGrid.load_preset(CGOL_frame.INITIAL_GAME_IDX)
        ...
        tbar = self.tbarMain
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_STEP,    self.IMAGE_STEP)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_WARP,    self.IMAGE_WARP)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_GRID,    self.IMAGE_GRID)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_SLOW,    self.IMAGE_SLOWER)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_FAST,    self.IMAGE_FASTER)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_TAKE,    self.IMAGE_TAKE_SNAP)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_RESTORE, self.IMAGE_RESTORE_SNAP)
        CGOL_frame.set_tool_image(tbar, self.TOOL_ID_ABOUT,   self.IMAGE_ABOUT)
        self.update_toolbar()

        # --- Configure game selector combo box ---
        games_list = self.pnlGrid.games_names_list
        self.cmbGame.Set(CGOL_frame.COMBO_ACTION_ITEMS + games_list)
        self.cmbGame.SetSelection(len(CGOL_frame.COMBO_ACTION_ITEMS) + CGOL_frame.INITIAL_GAME_IDX)
        ...
        self.sldSpeed.SetRange(0, CGOL_frame.SLIDER_MAX)
        self.sldSpeed.SetValue(CGOL_frame.SLIDER_INI)

        # --- Setup status bar ---
        self.stabStatusBar.SetFieldsCount(CGOL_frame.STATUS.N_FIELDS)
        self.stabStatusBar.SetStatusWidths(CGOL_frame.STATUS.FIELD_WIDTHS)
        ...
        self.timer_period_ms = CGOL_frame.get_timer_ms(CGOL_frame.SLIDER_INI)
        self.tmrGame.Start(self.timer_period_ms)

        # --- load a file if one was specified ---
        if filename:
            if self.pnlGrid.load_file(filename):
                self.__register_rules(self.pnlGrid.rules)
                self.__refresh_games_list()
                self.__refresh_tool_and_status(message=f"Game: {self.pnlGrid.name_of_game}")
            elif isinstance(filename, str):
                self.show_message(f'Unable to open file "{filename}"')
            else:
                self.show_message('Invalid file specified...')

        # --- finalize our initialization ---
    ...
    def show_message(self, message:str="", time_ms:int=0):
        ...
        self.GetStatusBar().SetStatusText(message, CGOL_frame.STATUS.IDX.FIELD_MESSAGE)

        if message:
            # set a new timer to kill the message
            if time_ms == 0: time_ms = CGOL_frame.MESSAGE_DISPLAY_MS
            self.timer_message = wx.CallLater(time_ms, self.GetStatusBar().SetStatusText, "", CGOL_frame.STATUS.IDX.FIELD_MESSAGE)
    ...
    def get_timer_ms(slider:int) -> int:
        ...
        Returns:
            _description_
        """
        T_MIN = CGOL_frame.TIMER_MIN_MS
        T_MAX = CGOL_frame.TIMER_MAX_MS
        R_TMR = T_MAX/T_MIN
        return int( T_MIN * R_TMR ** ((CGOL_frame.SLIDER_MAX - slider)/CGOL_frame.SLIDER_MAX))
    ...
    def update_toolbar(self):
        ...
        if self.pnlGrid.is_paused:
            CGOL_frame.set_tool_image(self.tbarMain, self.TOOL_ID_PLAY, self.IMAGE_PLAY)
            self.toolStep.Enable(True)
        else:
            CGOL_frame.set_tool_image(self.tbarMain, self.TOOL_ID_PLAY, self.IMAGE_PAUSE)
    ...
    def update_status_bar(self):
        ...
        if self.pnlGrid.is_paused:
            self.stabStatusBar.SetStatusText("Paused", CGOL_frame.STATUS.IDX.FIELD_STATE)
        else:
            self.stabStatusBar.SetStatusText("Running", CGOL_frame.STATUS.IDX.FIELD_STATE)

        # Live cell count field
        live_cells = self.pnlGrid.live_cells
        self.stabStatusBar.SetStatusText(f"Live cells: {live_cells}", CGOL_frame.STATUS.IDX.FIELD_LIVE)

        # Grid size field (clickable)
        rows, cols = self.pnlGrid.size()
        self.stabStatusBar.SetStatusText(f"{rows} x {cols}", CGOL_frame.STATUS.IDX.FIELD_GRID)

        # Rules field (clickable)
        self.stabStatusBar.SetStatusText(f"{self.pnlGrid.rules}", CGOL_frame.STATUS.IDX.FIELD_RULES)
    ...
    def on_click_fast(self, event:wx.CommandEvent):
        ...
        if pos > CGOL_frame.SLIDER_MAX: pos = CGOL_frame.SLIDER_MAX
        self.sldSpeed.SetValue(pos)
        tmr_ms = CGOL_frame.get_timer_ms(pos)
        self.timer_period_ms = tmr_ms
        self.is_reset_timer = True
        if pos == CGOL_frame.SLIDER_MAX:
    ...
    def on_status_click(self, event:wx.MouseEvent):
        ...
        for idx in range(CGOL_frame.STATUS.N_FIELDS):
        ...
        elif idx_field == CGOL_frame.STATUS.IDX.FIELD_RULES:
            self.pnlGrid.is_paused = True
        ...
        elif idx_field == CGOL_frame.STATUS.IDX.FIELD_MESSAGE:
    ...
    def __refresh_games_list(self):
        """Refreshes the games list shown in the toolbar
        """
        games_list = self.pnlGrid.games_names_list
        n_action_items = len(CGOL_frame.COMBO_ACTION_ITEMS)
        self.cmbGame.Set(CGOL_frame.COMBO_ACTION_ITEMS + games_list)
    ...
    def __refresh_tool_and_status(self, *, message:str|None=None):
        """Refreshes the toolbar and status bar, with an optional message

        Args:
            message: [None]   no message is displayed
                     [str]    displays the given message
        """
        self.update_toolbar()
        self.update_status_bar()
    ...
    def on_take_snap(self, event:wx.CommandEvent):
        ...
        if clip := self.pnlGrid.take_snapshot():
            CGOL_frame.copy_to_clipboard(clip)
    ...
    def on_restore_snap(self, event:wx.CommandEvent):
        ...
        Args:
            event: [wx.CommandEvent]
        """
        snapshot = CGOL_frame.copy_from_clipboard()
    ...
    def show_about_dialog(self):
        ...
        info.SetVersion(f"{CGOL_frame.VERSION}")
